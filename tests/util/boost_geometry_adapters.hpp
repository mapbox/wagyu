...
{
    static boost::iterator_range<typename mapbox::geometry::polygon<CoordinateType>::iterator> get(mapbox::geometry::polygon<CoordinateType>& p)
    {
        return boost::make_iterator_range(p.begin() + 1, p.end());
    }

    static boost::iterator_range<typename mapbox::geometry::polygon<CoordinateType>::const_iterator> get(mapbox::geometry::polygon<CoordinateType> const& p)
    {
        return boost::make_iterator_range(p.begin() + 1, p.end());
    }
};
