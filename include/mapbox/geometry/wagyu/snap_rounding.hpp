...
bool horizontals_at_top_scanbeam(T top_y,
    ...
    if (current_edge->bot.x < current_edge->top.x) {
        ...
        while (bnd_next != active_bounds.end() && 
               (*bnd_next == nullptr ||
               (*bnd_next)->current_x < (*bnd_curr)->current_x)) {
            if (*bnd_next != nullptr &&
                wround<T>((*bnd_next)->current_edge->top.y) != top_y &&
                wround<T>((*bnd_next)->current_edge->bot.y) != top_y) {
                mapbox::geometry::point<T> pt(wround<T>((*bnd_next)->current_x),
                                              top_y);
        ...
        if (bnd_curr != active_bounds.begin()) {
            ...
            while (bnd_curr != active_bounds.begin() &&
                   (*bnd_prev == nullptr ||
                   (*bnd_prev)->current_x > (*bnd_curr)->current_x)) {
                if (*bnd_prev != nullptr &&
                    wround<T>((*bnd_prev)->current_edge->top.y) != top_y &&
                    wround<T>((*bnd_prev)->current_edge->bot.y) != top_y) {
                    mapbox::geometry::point<T> pt(wround<T>((*bnd_prev)->current_x),
                                                  top_y);
                    add_to_hot_pixels(pt, manager);
                }
...
void process_hot_pixel_edges_at_top_of_scanbeam(T top_y,
    ...
    for (auto bnd = active_bounds.begin(); bnd != active_bounds.end();) {
        ...
        while (current_edge != current_bound.edges.end() &&
               current_edge->top.y == top_y) {
