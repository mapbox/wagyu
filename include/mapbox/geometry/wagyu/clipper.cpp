...
bool ClipperBase::AddPath(const Path& pg, PolyType PolyTyp, bool Closed) {
    ...
    for (;;) {
        ...
        if (E->Prev == E->Next)
            ...
            // the default is to merge adjacent collinear edges into a single
            // edge.
...
void Clipper::InsertLocalMinimaIntoAEL(const cInt botY) {
    ...
    while (PopLocalMinima(botY, lm)) {
        ...
        if (lb->OutIdx >= 0 && lb->PrevInAEL && lb->PrevInAEL->Curr.X == lb->Bot.X &&
            lb->PrevInAEL->OutIdx >= 0 && SlopesEqual(lb->PrevInAEL->Bot, lb->PrevInAEL->Top,
                                                      lb->Curr, lb->Top, m_UseFullRange) &&
...
void Clipper::ProcessHorizontal(TEdge* horzEdge) {
    ...
    for (;;) // loop through consec. horizontal edges
    {
        ...
        while (e) {
            // this code block inserts extra coords into horizontal edges (in
            // output
            // polygons) whereever maxima touch these horizontal edges. This
            // helps
            ...
            // Also break if we've got to the end of an intermediate horizontal
            // edge ...
            ...
            // OK, so far we're still in range of the horizontal Edge  but make
            // sure
            // we're at the last of consec. horizontals when matching with
            // eMaxPair
    ...
    if (horzEdge->NextInLML) {
        if (horzEdge->OutIdx >= 0) {
            ...
            // When StrictlySimple and 'horzEdge' is being touched by another
            // edge, then
...
bool IntersectListSort(IntersectNode* node1, IntersectNode* node2) {
    if (node2->Pt.Y != node1->Pt.Y) {
        ...
        return (node2->Edge1->WindCnt2 + node2->Edge2->WindCnt2) >
               (node1->Edge1->WindCnt2 + node1->Edge2->WindCnt2);
    }
}
//------------------------------------------------------------------------------
...
void Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY) {
    ...
    while (e) {
        ...
        if (IsMaximaEdge) {
            ...
            // When StrictlySimple and 'e' is being touched by another edge,
            // then
            // make sure both edges have a vertex here ...
...
bool GetOverlap(
    const cInt a1, const cInt a2, const cInt b1, const cInt b2, cInt& Left, cInt& Right) {
    if (a1 < a2) {
        if (b1 < b2) {
            Left = std::max(a1, b1);
            Right = std::min(a2, b2);
...
bool JoinHorz(
    OutPt* op1, OutPt* op1b, OutPt* op2, OutPt* op2b, const IntPoint Pt, bool DiscardLeft) {
...
void Clipper::FixupFirstLefts2(OutRec* InnerOutRec, OutRec* OuterOutRec) {
    ...
    // It's possible that these polygons now wrap around other polygons, so
    // check
    ...
    for (PolyOutList::size_type i = 0; i < m_PolyOuts.size(); ++i) {
        ...
        if (Poly2ContainsPoly1(outRec->Pts, InnerOutRec->Pts)) {
            ...
            if (outRec->IsHole == OuterOutRec->IsHole) {
                ...
                if (Area(outRec->Pts) == 0.0 &&
                    !Poly2ContainsPoly1(outRec->Pts, OuterOutRec->Pts)) {
                    outRec->IsHole = !outRec->IsHole;
                    outRec->FirstLeft = ParseFirstLeft(OuterOutRec->FirstLeft);
                    ReversePolyPtLinks(outRec->Pts);
                } else {
...
bool Clipper::FindIntersectLoop(std::unordered_multimap<int, OutPtIntersect>& dupeRec,
    ...
    for (auto it = range.first;
        ...
        if (FindIntersectLoop(dupeRec, iList, outRec_parent, idx_origin, itRec->Idx, visited,
                              orig_pt, it->second.op2)) {
            iList.emplace_front(idx_search, it->second);
            return true;
        }
    }
...
bool Clipper::FixIntersects(std::unordered_multimap<int, OutPtIntersect>& dupeRec,
    ...
    if (!found) {
        ...
        // Check for connection through chain of other intersections
        for (auto it = range.first;
             it != range.second && it != dupeRec.end() && it->first == outRec_search->Idx; ++it) {
            OutRec* itRec = GetOutRec(it->second.op2->Idx);
            if (itRec->Idx != outRec_search->Idx && op_origin_2->Pt != it->second.op2->Pt &&
                ...
                FindIntersectLoop(dupeRec, iList, outRec_parent, outRec_origin->Idx, itRec->Idx,
                                  visited, op_origin_2, it->second.op2)) {
                found = true;
