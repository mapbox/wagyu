...
active_bound_list_itr<T> process_horizontal_left_to_right(T scanline_y,
	...
	hot_pixel_set_itr<T> hp_itr;
	hot_pixel_set_itr<T> hp_end;
	bool has_hp = false;

    auto hot_pixels_itr = rings.hot_pixels.find(scanline_y);
    if (hot_pixels_itr != rings.hot_pixels.end()) {
		has_hp = true;
        hp_itr = hot_pixels_itr->second.begin();
        hp_end = hot_pixels_itr->second.end();
    }
    while (has_hp && hp_itr != hp_end && *hp_itr < (*horz_bound)->current_edge->bot.x) {
        ++hp_itr;
    }
    ...
    while (bnd != active_bounds.end()) {
        ...
        while (has_hp && hp_itr != hp_end && *hp_itr < std::llround((*bnd)->current_x) &&
               *hp_itr < (*horz_bound)->current_edge->top.x) {
            if ((*horz_bound)->ring && !is_open) {
                add_point_to_ring(
                    *(*horz_bound),
                    mapbox::geometry::point<T>(*hp_itr, scanline_y), rings);
            }
            ++hp_itr;
        }
...
active_bound_list_itr<T> process_horizontal_right_to_left(T scanline_y,
	...
	hot_pixel_set_rev_itr<T> hp_itr;
	hot_pixel_set_rev_itr<T> hp_end;
	bool has_hp = false;

    auto hot_pixels_itr = rings.hot_pixels.find(scanline_y);
    if (hot_pixels_itr != rings.hot_pixels.end()) {
		has_hp = true;
        hp_itr = hot_pixels_itr->second.rbegin();
        hp_end = hot_pixels_itr->second.rend();
    }
    while (has_hp && hp_itr != hp_end && *hp_itr > (*horz_bound)->current_edge->bot.x) {
        ++hp_itr;
    }
    ...
    while (bnd != active_bounds.rend()) {
        ...
        while (has_hp && hp_itr != hp_end && *hp_itr > std::llround((*bnd)->current_x) &&
               *hp_itr > (*horz_bound)->current_edge->top.x) {
            if ((*horz_bound)->ring && !is_open) {
                add_point_to_ring(*(*horz_bound), mapbox::geometry::point<T>(*hp_itr, scanline_y), rings);
            }
            ++hp_itr;
        }
    ...
    if ((*horz_bound)->ring && !is_open) {
        while (has_hp && hp_itr != hp_end && *hp_itr > (*horz_bound)->current_edge->top.x) {
            add_point_to_ring(*(*horz_bound), mapbox::geometry::point<T>(*hp_itr, scanline_y), rings);
            ++hp_itr;
        }
    }
