                       ...
                       edge_ptr<T> active_edges)
{
    ...
    if (!e)
    {
        ...
        {
            fill_type pft =
                (edge.PolyTyp == polygon_type_subject ? subject_fill_type
                                                      : clip_fill_type);
    ...
    else
    {
        ...
        if (e->winding_count * e->winding_delta < 0)
        {
            ...
            else
            {
                ...
                edge.winding_count =
                    (edge.winding_delta == 0 ? 1 : edge.winding_delta);
            }
        }
        else
        {
            ...
            {
                edge.winding_count =
                    (e->winding_count < 0 ? e->winding_count - 1
                                          : e->winding_count + 1);
            }
                                  ...
                                  fill_type clip_fill_type)
{
    ...
    while (pop_local_minima(botY, lm, current_local_min, minima_list)) {
        ...
        if (!lb)
        {
            ...
            set_winding_count(*rb, cliptype, subject_fill_type, clip_fill_type,
                              active_edges);
            if (is_contributing(*rb, cliptype, subject_fill_type,
                                clip_fill_type))
            {
                p1 = add_point(rb, rb->bot, rings);
                edge_ptr<value_type> eprev = rb->prev_in_AEL;
                if (rb->index >= 0 && rb->winding_delta != 0 && eprev &&
                    eprev->index >= 0 && eprev->curr.x == rb->curr.x &&
                    eprev->winding_delta != 0)
                {
                    add_point(eprev, rb->curr, rings);
                }
                edge_ptr<value_type> enext = rb->next_in_AEL;
                if (rb->index >= 0 && rb->winding_delta != 0 && enext &&
                    enext->index >= 0 && enext->curr.x == rb->curr.x &&
        ...
        else if (!rb)
        {
            ...
            set_winding_count(*lb, cliptype, subject_fill_type, clip_fill_type,
                              active_edges);
                                ...
                                clip_fill_type))
            {
                ...
                if (lb->index >= 0 && lb->winding_delta != 0 && enext &&
                    enext->index >= 0 && enext->curr.x == lb->curr.x &&
                    enext->winding_delta != 0)
                {
        ...
        if (p1 && is_horizontal(*rb) && !ghost_joins.empty() &&
            rb->winding_delta != 0)
        {
            for (auto jr = ghost_joins.begin(); jr != ghost_joins.end(); ++jr) {
                ...
                if (horizontal_segments_overlap(jr->point1->x, jr->off_point.x,
                                                rb->bot.x, rb->top.x))
                {
        ...
        if (lb->index >= 0 && lb->prev_in_AEL &&
            ...
            slopes_equal(lb->prev_in_AEL->bot, lb->prev_in_AEL->top, lb->curr,
                         lb->top) &&
            lb->winding_delta != 0 && lb->prev_in_AEL->winding_delta != 0)
        {
            point_ptr<value_type> p2 =
                add_point(lb->prev_in_AEL, lb->bot, rings);
        ...
        if (lb->next_in_AEL != rb)
        {
            if (rb->index >= 0 && rb->prev_in_AEL->index >= 0 &&
                slopes_equal(rb->prev_in_AEL->curr, rb->prev_in_AEL->top,
                             rb->curr, rb->top) &&
                rb->winding_delta != 0 && rb->prev_in_AEL->winding_delta != 0)
            {
                point_ptr<value_type> p2 =
                    add_point(rb->prev_in_AEL, rb->bot, rings);
