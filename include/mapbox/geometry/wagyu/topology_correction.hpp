...
void correct_orientations(ring_manager<T>& manager) {
    for (auto& r : manager.rings) {
        ...
        remove_spikes_in_polygons(&r, manager);
        r.area = std::numeric_limits<double>::quiet_NaN();
    }
}
...
void correct_self_intersection(point_ptr<T> pt1,
    ...
    std::size_t size_1 = 0;
    std::size_t size_2 = 0;
    double area_1 = area_from_point(pt1, size_1);
    double area_2 = area_from_point(pt2, size_2);
...
bool find_parent_in_tree(ring_ptr<T> r,
    ...
    if (poly2_contains_poly1(r, possible_parent)) {
        assign_as_child(r, possible_parent, manager);
        return true;
    }
    return false;
}
...
void assign_new_ring_parents(ring_manager<T>& manager,
    ...
    double original_ring_area = area(original_ring);
    ...
    if (new_rings.size() == 1) {
        double new_ring_area = area(new_rings.front());
    ...
    std::stable_sort(new_rings.begin(), new_rings.end(),
        [] (ring_ptr<T> const& r1, ring_ptr<T> const& r2) {
            return std::fabs(area(r1)) > std::fabs(area(r2));
        });

    for (auto r_itr = new_rings.begin(); r_itr != new_rings.end(); ++r_itr) {
        double new_ring_area = area(*r_itr);
        bool new_positive = new_ring_area > 0.0;
        ...
        if (same_orientation) {
            ...
            if (!find_parent_in_tree(*r_itr, original_ring, manager)) {
                // If we didn't find a parent here, something is wrong
                // and we failed somewhere so lets throw an exception
...
void process_single_intersection(std::unordered_multimap<ring_ptr<T>, point_ptr_pair<T>>& connection_map,
    ...
    if (op_origin_1 == nullptr || op_origin_2 == nullptr) {
        ...
        if (ring_is_hole(ring_origin)) {
            ring_new->parent = ring_origin;
            ring_new->parent->children.push_back(ring_new);
            fixup_children_complex(ring_origin, ring_new, grand_children, manager);
            fixup_children_complex(ring_parent, ring_new, grand_children, manager);
        } else {
            ring_new->parent = ring_origin->parent;
            if (ring_new->parent == nullptr) {
                manager.children.push_back(ring_new);
            } else {
                ring_new->parent->children.push_back(ring_new);
            }
            fixup_children_complex(ring_origin, ring_new, grand_children, manager);
