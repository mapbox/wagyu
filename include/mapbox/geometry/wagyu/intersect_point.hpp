    ...
    if (std::fabs(Edge1.dx - Edge2.dx) < std::numeric_limits<double>::epsilon()) {
        ...
        if (is_horizontal(Edge2)) {
            ...
            if (Edge2.bot.x == Edge1.bot.x) {
                ip.y =
                    static_cast<value_type>(std::round(static_cast<double>(ip.x) / Edge2.dx + b2));
            } else if (Edge2.bot.x < Edge1.bot.x) {
                ip.y = static_cast<value_type>(
                    std::round((static_cast<double>(ip.x) - 0.5) / Edge2.dx + b2));
            } else {
                ip.y = static_cast<value_type>(
                    std::round((static_cast<double>(ip.x) + 0.5) / Edge2.dx + b2));
            }
        ...
        if (is_horizontal(Edge1)) {
            ...
            if (Edge1.bot.x == Edge2.bot.x) {
                ip.y =
                    static_cast<value_type>(std::round(static_cast<double>(ip.x) / Edge1.dx + b1));
            } else if (Edge1.bot.x < Edge2.bot.x) {
                ip.y = static_cast<value_type>(
                    std::round((static_cast<double>(ip.x) - 0.5) / Edge1.dx + b1));
            } else {
                ip.y = static_cast<value_type>(
                    std::round((static_cast<double>(ip.x) + 0.5) / Edge1.dx + b1));
            }
        ...
        do {
            ...
            value_type y2 = nearest_along_y_dimension(Edge2.bot.x, Edge2.bot.y, Edge1.bot.y, ip.x,
                                                      ip.y, by2, Edge2.dx);
            value_type x1 = nearest_along_x_dimension(Edge1.bot.x, Edge1.bot.y, Edge2.bot.x, ip.x,
                                                      ip.y, bx1, Edge1.dx);
